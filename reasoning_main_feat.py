"""
adding node type feature for tgbl-wiki dataset
"""
...
def make_user_prompt(src, ts, nbr_tracker=None):
    if (nbr_tracker is not None):
        ...
        user_prompt += f"Please predict the most likely `Page Node` for `User Node` {src} at `Timestamp` {ts}."
    else:
        user_prompt = (
            f"Predict the next interaction for User node {src} at time {ts},"
        )
    return user_prompt
...
def make_system_prompt(background_rows, instruct_strs, answer_strs, use_icl=True):
    ...
    #     ) # system prompt
    system_prompt = (
            f"You are an expert temporal graph learning agent. Your task is to predict the next interaction (i.e. Page Node) given the `User Node` and `Timestamp`.\n\n" 
            f"Description of the temporal graph is provided below, where each line is a tuple of (`User Node`, `Page Node`, `Timestamp`).\n\nTEMPORAL GRAPH:\n"
        ) # system prompt
...
for i in range(test_rows.shape[0]):
    ...
    if (i >= start_idx):
        ...
        if (reasoning != "Error in generating response"):
            # valid_cots += 1
            answer_json = [{"predicted node": predicted_dst, "true_answer":int(test_rows[i][1]) ,"explanation": reasoning}]
        else:
            answer_json = []
            error_prompts += 1
